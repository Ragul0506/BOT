...
generate_bill_pdf() accepts an optional ``language`` parameter:
  'ta' (default) — Tamil shop name + bilingual headers
  'en'           — English-first headers (still bilingual in totals row)
"""
...
def generate_bill_pdf(items: list[dict], language: str = "ta") -> str:
    """Build a PDF invoice and return its temp-file path (caller must delete).

    Args:
        items:    List of {'item', 'qty', 'rate'} dicts.
        language: 'ta' for Tamil-first headers, 'en' for English-first headers.
    """
    ...
    s_td_c = _ps("TDC", font, fontSize=9, alignment=1)
    s_td_l = _ps("TDL", font, fontSize=9, alignment=0)
    ...
    if language == "en":
        story.append(_p("VILLAGE GROCERY STORE", s_shop))
        story.append(_p("கிராம மளிகை கடை", s_sub))
    else:
    ...
    # Column header label depends on language
    item_col_label = "Item / பொருள்" if language == "en" else "பொருள் / Item"

    rows: list[list] = [[
        _p("S.No", s_th),
        _p(item_col_label, s_th),
        _p("Qty", s_th),
        _p("Rate (&#8377;)", s_th),
        _p("Amount (&#8377;)", s_th),
    ]]
    ...
    # Total row — always bilingual
    total_label = "Total / மொத்தம்" if language == "en" else "மொத்தம் / Total"
    rows.append([
        _p("", s_total),
        _p("", s_total),
        _p("", s_total),
        _p(total_label, s_total),
    ...
    doc.build(story)
    logger.info(
        "PDF ready: %s  |  lang=%s  |  Grand total: %.2f",
        tmp.name, language, grand_total,
    )
    return tmp.name
